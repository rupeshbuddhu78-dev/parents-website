...
// Proxy Cloudinary call/audio so browser <audio> can play (no CORS / format issues)
app.get('/api/play-audio', async (req, res) => {
    try {
        let src = String(req.query.url || '').trim();
        if (!src) return res.status(400).json({ error: 'url required' });
        // Only allow Cloudinary HTTPS URLs
        let parsed;
        try { parsed = new URL(src); } catch (e) { return res.status(400).json({ error: 'bad url' }); }
        if (parsed.protocol !== 'https:' || !/cloudinary\.com$/i.test(parsed.hostname) && !/\.cloudinary\.com$/i.test(parsed.hostname)) {
            return res.status(400).json({ error: 'only cloudinary urls allowed' });
        }
        // Prefer browser-friendly delivery: force mp3/mp4 when path is video/upload
        if (parsed.pathname.includes('/video/upload/') && !parsed.pathname.includes('/f_')) {
            parsed.pathname = parsed.pathname.replace('/video/upload/', '/video/upload/f_mp3/');
            src = parsed.toString();
        }
        ...
        res.setHeader('Content-Type', ct.includes('video') ? 'audio/mp4' : ct);
        ...
        console.error('play-audio proxy', e.message);
